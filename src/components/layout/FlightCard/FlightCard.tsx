...
interface FlightCardProps {
    flight: Flight;
    onSelect?: (selectedFlight: Flight & { selectedClass: FlightClass; price: number }) => void;
}
...
const FlightCard: React.FC<FlightCardProps> = ({flight, onSelect}) => {
    ...
    const priceBoxes = Object.entries(flight.prices).map(([classType, price]) => {
        const theme = flightClassThemes[classType as keyof typeof flightClassThemes];

        return (
            <Grid size={4} key={classType}>
                <Paper
                    elevation={2}
                    sx={{
                        p: 1.2,
