    ...
    const GenericFilter: React.FC<GenericFilterProps> = ({
        ...
        const hasActiveFilters = useMemo(() => {
            return Object.entries(state).some(([filterId, value]) => {
        ...
        const activeFilterCount = useMemo(() => {
            return Object.entries(state).reduce((count, [_, value]) => {
                ...
                <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", mb: 2 }}>
                    ...
                    <Box sx={{ display: "flex", gap: 1, alignItems: "center" }}>
                        ...
                        {variant !== "sidebar" && showClearButton && hasActiveFilters && (
                            <Button size={dense ? "small" : "medium"} startIcon={<ClearIcon />} onClick={handleClear} sx={{ textTransform: 'none' }}>
                                Clear
                            </Button>
                        )}
                    </Box>
                            ...
                            <AccordionSummary expandIcon={<ExpandMoreIcon />} sx={{ px: 0 }}>
                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, minWidth: 0 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, overflow: 'hidden', whiteSpace: 'nowrap' }}>
                                        {group.icon}
                                        <Typography sx={{ fontWeight: 500, overflow: 'hidden', textOverflow: 'ellipsis' }}>{group.label}</Typography>
                                    </Box>
                                </Box>
                            </AccordionSummary>
            ...
            case 'drawer':
                return (
                    <Drawer anchor="right" open PaperProps={{ sx: { width: isMobile ? '100%' : 400 } }}>
                        <Box sx={{ p: dense ? 2 : 3, display: 'flex', flexDirection: 'column', height: '100%' }}>{renderContent()}</Box>
                    </Drawer>
                );
    ...
    const FilterSection: React.FC<FilterSectionProps> = ({ filter, value, onChange, dense }) => {
        if (isCheckboxFilter(filter)) {
            return <CheckboxFilterSection filter={filter} value={value as string[] | number[]} onChange={onChange} dense={dense} />;
        }
        if (isRangeFilter(filter)) {
            return <RangeFilterSection filter={filter} value={value as [number, number]} onChange={onChange} dense={dense} />;
        }
        ...
        if (isDateRangeFilter(filter)) {
            return <DateRangeFilterSection filter={filter} value={value as [Date, Date]} onChange={onChange} dense={dense} />;
        }
        return null;
    };
