...
const PassengerStep: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const subscription = watch((_, {name}) => {
            ...
            const match = name.match(/^passengers\.(\d+)\./);
            if (match) {
                const index = parseInt(match[1], 10);

                const timer = setTimeout(async () => {
                    const isComplete = await trigger(`passengers.${index}`);
                    setPassengerCompletion(prev => ({...prev, [index]: isComplete}));
                }, 500);
        ...
        const subscription = watch((data) => {
            try {
                const serializedPassengers = data.passengers?.map((p) => ({
                    ...p,
                    dateOfBirth:
                        p?.dateOfBirth instanceof Date ? p.dateOfBirth.toISOString() : p?.dateOfBirth || '',
                    passportExpiry:
                ...
                setError({message: 'Failed to update passenger information'});
    ...
    const handleScanComplete = useCallback(
        (scannedData: ScannedPassportData, passengerIndex: number) => {
            try {
                ...
                setTimeout(() => {
                    trigger(`passengers.${passengerIndex}`);
                }, 100);
            } catch (err) {
                setError({message: 'Failed to process passport scan data'});
    ...
    const handleNextPassenger = useCallback(async () => {
        const isCurrentValid = await trigger(`passengers.${currentPassengerIndex}`);
        if (!isCurrentValid) {
            setError({
    ...
    }, [currentPassengerIndex, passengerCount, trigger]);
                                ...
                                <Grid container spacing={2}>
                                    <Grid size={6}>
                                        <FormInput
                                            ...
                                            disabled={isLoading}
                                            showAutofillWarning={shouldShowWarning(
